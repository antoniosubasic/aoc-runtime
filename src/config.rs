...
pub struct Config {
    ...
    #[serde(
        skip_serializing,
        skip_deserializing,
        default = "Config::default_regex"
    )]
    path_regex: Regex,
}

impl Config {
    // used by serde for default initialization of field
        ...
        // algorithm to escape template path and insert regex patterns for parameter extraction
        {
            ...
            for range in &param_locations {
                if previous < range.start {
                    pattern.push_str(&regex::escape(&config.template_path[previous..range.start]));
                }
            ...
            config.path_regex = Regex::new(&pattern)?;
        }
    ...
    pub fn build(&mut self, args: &mut Args) -> Result<()> {
        ...
        if let Some(captures) = self.path_regex.captures(&working_directory) {
            if captures.len() == 4
